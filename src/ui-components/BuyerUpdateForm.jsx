...
import { getBuyer } from "../graphql/queries";
import { updateBuyer } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function BuyerUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = buyerRecord
      ? { ...initialValues, ...buyerRecord }
      : initialValues;
    setBuyerType(cleanValues.buyerType);
  ...
  const [buyerRecord, setBuyerRecord] = React.useState(buyerModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getBuyer.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getBuyer
        : buyerModelProp;
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateBuyer.replaceAll("__typename", ""),
            variables: {
              input: {
                id: buyerRecord.id,
                ...modelFields,
    ...
    >
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || buyerModelProp)}
